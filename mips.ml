...
let zero = "$zero" (*ajout du registre 0*)
let t0 = "$t0"
let t1 = "$t1"
let a0 = "$a0"
let v0 = "$v0"
...
let div r1 r2      = S(sprintf "  div  %s, %s"     r1 r2)
let mflo r         = S(sprintf "  mflo %s"         r)
let mfhi r         = S(sprintf "  mfhi %s"         r)
let neg r1 r2      = S(sprintf "  neg  %s, %s"     r1 r2)
...
let asciiz s = S(sprintf "  .asciiz \"%s\"" s) (* ajout de \*)
